...
async def play_add_handler(self, message, cmd, args):
    ...
    if args:
        ...
        if any(domain in url for domain in ["soundcloud.com", "mixcloud.com", "nts.live", "bandcamp.com"]):
            await mpd.tracklist.add(url)
            await message.channel.send(f"Added to jukebox: {url}")
        else:
            await message.channel.send("Please provide a valid link from SoundCloud, Mixcloud, NTS, or Bandcamp")
    else:
