...
async def tagged_handler(self, message, cmd, args):
    ...
    try:
        if not args:
            ...
            if tagged_items:
                await message.channel.send(f"Found {len(tagged_items)} items tagged with '{args}'")
                # Send a few examples
...
async def untag_handler(self, message, cmd, args):
    ...
    try:
        if not args:
            await message.channel.send("sorry, ambiguous input, please use !untag url tagname")
        else:
            parts = args.split()
            if len(parts) != 2:
                await message.channel.send("sorry, ambiguous input, please use !untag url tagname")
...
async def info_handler(self, message, cmd, args):
    ...
    try:
        if not args:
            await message.channel.send("use like '!info woi' to get info about tags and urls")
        else:
