            ...
            async with ClientSession() as s:
                ...
                if r:
                    ...
                    if not chuntfm_queried_show:
                        chuntfm_queried_show = (
                            f"No upcoming shows found matching '{args}'"
                        )
                else:
    ...
    try:
        async with ClientSession() as s:
            ...
            if r:
                ...
                for show in schedule_json:
                    show_date = datetime.fromisoformat(show["startTimestamp"]).strftime(
                        "%Y-%m-%d"
                    )
                ...
                if shows_for_date:
                    ...
                    date_label = (
                        date_labels[date_offset_days]
                        if abs(date_offset_days) <= 1
                    ...
                    for show in shows_for_date:
                        ...
                        schedule_text += (
                            f"{show['startTimeUK']} {show['title']} | {clean_desc} "
                        )
                else:
