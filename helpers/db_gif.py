...
class DB_GIF(AsyncDB):
    ...
    async def create_tag(self, intag):
        ...
        await self.conn.commit()
        result_tag_id = cursor.lastrowid
    ...
    async def remove_tag_by_tag_id(self, tag_id):
        await self.conn.execute("DELETE FROM tag_table WHERE id = ?", (tag_id,))
        await self.conn.commit()

    async def remove_object_by_object_id(self, object_id):
        await self.conn.execute("DELETE FROM object_table WHERE id = ?", (object_id,))
        await self.conn.commit()
